...
Topics subscribed:
  ...
  /oak/imu/data                  (utils/IMU)            → deviceRotation to server
...
sys.path.insert(0, 'src/input/src')
...
class TrafficBridge(Node):
    ...
    def __init__(self, queue_list: dict, engine: threadTrafficCommunication):
    ...
    def _drain(self):
        ...
        while self._running and rclpy.ok():
            ...
            ros_msg.pos_a = float(value.get('x', 0.0))  # already in metres
    ...
    def _imu_cb(self, msg: IMU):
        ...
        self._send('deviceRotation', [yaw_deg])
    ...
    def _send(self, command: str, values: list):
        """Send directly via tcp_factory (bypasses sharedMem, simpler)."""
        tcp = self._engine.tcp_factory
        payload = {'reqORinfo': 'info', 'type': command}
        if len(values) > 0: payload['value1'] = values[0]
        if len(values) > 1: payload['value2'] = values[1]
        if len(values) > 2: payload['value3'] = values[2]
        tcp.send_data_to_server(payload)
...
def main(args=None):
    ...
    node = TrafficBridge(queue_list, engine)
