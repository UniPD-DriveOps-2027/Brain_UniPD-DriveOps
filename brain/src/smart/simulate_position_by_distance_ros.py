...
if __name__ == "__main__":
    ...
    print("- Press 'A' to decrease speed")
    print("- Press 'D' to increase speed")
    print("- Press 'Q' to quit")
    
    while True:
        # Update position based on simulated movement
        distance = SIM_SPEED * DT
        position, idx = tracker.update_from_distance(distance)
        ...
        if key == ord('a'):
            ...
            SIM_SPEED = min(2.0, SIM_SPEED + 0.25)
        elif key == ord('q'):
            break
    
    cv2.destroyAllWindows()
    print(f"Simulation completed. Final position: {position}")