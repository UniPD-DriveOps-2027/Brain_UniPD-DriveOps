...
class Brain:
    ...
    def follow_lane(self):
        ...
        e2_sw, e3_sw, pt_sw = self.detect.detect_lane_sliding_window(self.car.frame, SHOW_IMGS)
        if e2_sw is not None:
            alpha = 0.7  # weight for NN; 1-alpha for sliding window
            e2 = alpha * e2 + (1 - alpha) * e2_sw
            e3 = alpha * e3 + (1 - alpha) * e3_sw
            point_ahead = alpha * point_ahead + (1 - alpha) * pt_sw
