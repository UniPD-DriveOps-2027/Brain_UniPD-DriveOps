#!/usr/bin/env python3
# Purpose: Forward vehicle/Brain metrics to a Unix-domain dashboard socket.
# Inputs: ROS telemetry topics and socket configuration.
# Outputs: Serialised metrics written to a Unix socket.
...
from brain_interfaces.msg import Conditions
...
class MetricsToSocket(Node):
    ...
    def _imu_cb(self, msg: Imu) -> None:
        ...
        ) + YAW_GLOBAL_OFFSET) % 360.0
        self._cache['YAW'] = round(float(yaw), 2)
        if 45 <= yaw < 135:
