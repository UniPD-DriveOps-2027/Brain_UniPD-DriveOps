#!/usr/bin/python3
# Purpose: Define the hardware-independent vehicle data and command contract.
# Inputs: Sensor/localisation updates and requested speed, steering, stop, or position actions.
# Outputs: Normalised vehicle state plus abstract commands implemented by adapters.
...
class Automobile_Data():
    def __init__(self,
        ...
        # The legacy EKF path is disabled in update_estimated_state(). Keep
        # its slot explicit without forcing CasADi/filterpy onto every runtime.
        self.ekf = None
    ...
    # UNUSED: Automobile_Data.update_estimated_state has no production caller.
    ...
    #
        # if DT > 0 and \
                # len(self.past_encoder_distances) > \
                # BUFFER_PAST_MEASUREMENTS_LENGTH-1:
            # self.new_gps_sample_arrived = True
...
# USED ONLY IN 2024 
    # UNUSED: Automobile_Data.decide_yaw_start has no production caller.
