#!/usr/bin/python3
# Purpose: Provide coordinate transforms and legacy navigation geometry helpers.
# Inputs: Vehicle poses, route points, headings, and controller state.
# Outputs: Distances, transformed points, path-tracking commands, and visualisation data.
...
# UNUSED: pix2mL has no remaining production caller.
# def pix2mL(pix):
...
# UNUSED: pix2mR has no production caller.
# def pix2mR(pix):
    # # pixel to meters (right frame)
    # return (pix2mL(pix)) @ np.array([[1.0, 0.0], [0.0, -1.0]]) + np.array([0, 15.0])
...
# UNUSED: draw_bounding_box has no production caller.
...
# UNUSED: wrap_detection has no production caller.
...
# UNUSED: my_softmax has no production caller.
# def my_softmax(x):
    # return np.exp(x) / np.sum(np.exp(x), axis=0)


# UNUSED: project_curvature has no production caller.
...
# UNUSED: project_stopline has no production caller.
...
# UNUSED: m2pix has no production caller.
# def m2pix(m):
    # return np.int32(m*const_verysmall)


# UNUSED: pix2m has no production caller.
# def pix2m(pix):
    # return 1.0*pix/const_verysmall


# UNUSED: yaw2world has no production caller.
# def yaw2world(angle):
    # return -(angle + np.pi/2)


# UNUSED: world2yaw has no production caller.
# def world2yaw(angle):
    # return -angle - np.pi/2
...
# UNUSED: show_car has no production caller.
# def show_car(track, car, show=True):
    # if show:
        # track1 = track.copy()
        ...
        # #if len(brain.path_planner.path) > 0:
            # #cv.circle(track1, mR2pix(brain.path_planner.path[int(brain.car_dist_on_path*100)]), 10, (150, 50, 255), 3)
        # cv.imshow('Map', track1)
        # cv.waitKey(1)


# UNUSED: show_camera has no production caller.
# def show_camera(car, show=True):
    # if show:
...
def draw_event(path_planner, path_ahead, draw):
    if draw:
        for p in path_ahead:
            cv.circle(path_planner.map, mR2pix(p), 10, (200, 150, 0), 5)


def determine_intersection_direction(brain, local_path_cf):
    point_ahead = local_path_cf[-1]
    yaw_error = np.arctan2(point_ahead[1], point_ahead[0])
    print("TOTAL YAW ERROR: ", yaw_error)
    if yaw_error < -0.3:
        brain.curr_state.var4 = "right"
    elif yaw_error > 0.3:
        brain.curr_state.var4 = "left"
    else:
        brain.curr_state.var4 = "forward"
...
def navigate_roundabout(brain, idx_point_ahead, max_idx, show):
    ...
    if max_idx - idx_point_ahead < 10:
        ...
        e3, _ = brain.detect.detect_roundabout_about(brain.car.frame, show_ROI=show)
        brain.curr_state.var4 = 0.3 * brain.curr_state.var4 + 0.7 * e3
        output_speed, output_angle = brain.controller_sp.get_control_speed(0.0, brain.curr_state.var4, 1.4*brain.curr_state.var4)
    else:
        print("\nABOUT ABOUTABOUT 3\n")
        e3, _ = brain.detect.detect_roundabout_about(brain.car.frame,show_ROI=show)
        brain.curr_state.var4 = 0.3 * brain.curr_state.var4 + 0.7 * e3
...
#     #print(f'output_speed: {output_speed:.2f}, output_angle: {np.rad2deg(output_angle):.2f}')
    ...
    STEER_RIGHT = 12.0   # degrees — tune on track
...
# UNUSED: switch_lane_check has no production caller.
# def switch_lane_check(closest_node, meas):
    # with open(data_path('gps_angles.pkl'), 'rb') as fp:
        # node_dict = pickle.load(fp)
    # with open(data_path('highway_dict.pkl'), 'rb') as fp:
