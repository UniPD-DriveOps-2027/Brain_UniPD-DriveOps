#!/usr/bin/python3
# Purpose: Build shortest routes and dense paths from the directed track graph.
# Inputs: Checkpoint node IDs, graph/data assets, and optional vehicle pose.
# Outputs: Route nodes, interpolated Nx2 path, events, and route metadata.

from brain_core.common.constants import SHOW_IMGS
...
class PathPlanning():
    def __init__(self, map_img):
        ...
        self.prev_index = 0
        graph_path = data_path('final_graph.graphml')
        ...
        intersection_cen_path = data_path('int_mid.txt')
        intersection_in_path = data_path('int_in.txt')
        intersection_out_path = data_path('int_out.txt')
        ra_path = data_path('ra_mid.txt')
        ra_enter_path = data_path('ra_in.txt')
        ra_exit_path = data_path('ra_out.txt')
        highway_nodes_path = data_path('hw.txt')
        no_lane_nodes_path = data_path('no_lane.txt')
        ...
        # Event points
        event_points_path = data_path('event_points.txt')
        event_types_path = data_path('event_types.txt')
    ...
    # UNUSED: PathPlanning.generate_path_passing_through has no production caller.
    ...
    # UNUSED: PathPlanning.get_path_ahead has no production caller.
    ...
    # UNUSED: PathPlanning.get_closest_stopline has no production caller.
    ...
    # UNUSED: PathPlanning.print_path_info has no production caller.
    ...
    # UNUSED: PathPlanning.get_length has no production caller.
    ...
    # UNUSED: PathPlanning.get_path has no production caller.
    # def get_path(self):
        # return self.path

    # UNUSED: PathPlanning.print_navigation_instructions has no production caller.
    ...
    # UNUSED: PathPlanning.is_dotted has no production caller.
