#!/usr/bin/python3
# Purpose: Compute legacy camera-error steering corrections.
# Inputs: Lane lateral/heading errors and controller timing/state.
# Outputs: Clamped steering-angle command.
...
class Controller():
    ...
    def get_control(self, e2, e3, curv, desired_speed, gains=None, no_lane=False):
        ...
        if no_lane:
            print('Debug: Using k3_NL')
            k3 = self.k3_NL

        # yaw error (e3), proportional term
        # POINT_AHEAD_CM/100.0
        # distance point ahead, matched with lane_detection
    ...
    # UNUSED: Controller.get_training_control has no production caller.
    ...
    #
        ...
        # # y error in the body frame
        # e2 = ex * np.cos(car.yaw) + ey * np.sin(car.yaw)
    #
        # # get point ahead
        # assert len(path_ahead) > max(NUM_POINTS * SEQ_POINTS_INTERVAL,
               # int(100*self.dist_point_ahead)), \
            # f"path_ahead is not long enough, len: {len(path_ahead)}"
        # point_ahead = path_ahead[
                # min(int(100*self.dist_point_ahead), len(path_ahead)-1), :]
        # # translate to car coordinates
        # point_ahead = hf.to_car_frame(point_ahead, car, return_size=2)
    #
        # self.seq_points_ahead = [path_ahead[(i)*SEQ_POINTS_INTERVAL, :]
                                 # for i in range(NUM_POINTS+1)]
        # self.seq_points_ahead = [hf.to_car_frame(p, car, return_size=2)
                                 # for p in self.seq_points_ahead]
        # # yaws should be changed: they should be calculated wrt the car's
        # # not wrt to each other
        # self.seq_yaws_ahead = [np.arctan2(self.seq_points_ahead[i][1],
                                          # self.seq_points_ahead[i][0])
                               # for i in range(NUM_POINTS)]
    #
        # alpha = np.arctan2(point_ahead[1], point_ahead[0]+L/2)
        # output_speed, output_angle = self.get_control(e2,
                                                      # alpha,
                                                      # curvature_ahead,
                                                      # vd)
    #
        # # add noise for training
        # output_angle = output_angle + self.get_random_noise(std=self.noise_std)
        # return output_speed, output_angle, point_ahead

    # UNUSED: Controller.pack_input_data has no remaining production caller.
    # def pack_input_data(self):
        # data = [0, 0, 0, 0]
        # xd, yd, yawd, curv, path_ahead, stopline_x, stopline_y, stopline_yaw, \
            # (state, next, action, dist, _, _, _) = self.curr_data
        # if action == "straight":
            # data[0] = 1
        # elif action == "left":
            # data[1] = 1
    ...
    # UNUSED: Controller.pack_regression_labels has no remaining production caller.
    # def pack_regression_labels(self, bb_const=1000.0):
        ...
        # if dist is None:
            # dist = 10
        # # add errors
        # reg_label = [self.e2, self.e3, curv, dist,
                     # stopline_x, stopline_y, stopline_yaw]
    ...
    # UNUSED: Controller.pack_classification_labels has no remaining production caller.
    ...
    # UNUSED: Controller.get_action_vector has no production caller.
    ...
    # UNUSED: Controller.get_random_noise has no remaining production caller.
    # def get_random_noise(self, std=np.deg2rad(20)):
        # if self.cnt == self.noise_reset:
            # self.cnt = 0
            # self.noise = np.random.normal(0, std)
            # self.noise_reset = np.random.randint(NOISE_RESET_MEAN -
                                                 # NOISE_RESET_STD,
                                                 # NOISE_RESET_MEAN +
                                                 # NOISE_RESET_STD)
        # self.cnt += 1
    ...
    # UNUSED: Controller.save_data has no production caller.
