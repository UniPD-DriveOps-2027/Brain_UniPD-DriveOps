# Purpose: Implement the autonomous driving state machine and its behaviours.
# Inputs: AutomobileData, Detection, PathPlanning, controllers, checkpoints, and environment state.
# Outputs: Vehicle commands, state transitions, route progress, and runtime checkpoint state.
...
def checkpoint_route_complete(command):
    """Return whether a path-following leg has reached its endpoint.

    Keeping this decision independent from the event sequence is important for
    checkpoint-only testing: mapped stop-line and crosswalk events may precede
    the end event, but they must not prevent the controller from completing the
    current graph route.
    """
    return bool(
        command is not None
        and command.has_path
        and (
            command.route_complete
            or (
                command.remaining_m <= 0.12
                and command.distance_to_goal_m <= 0.30
            )
        )
    )
...
STOPLINE_STOP_DISTANCE = 0.1      # 0.15 simulation       # 0.1 #in the true map
...
class Brain:
    def __init__(self,
        ...
        self.events = []
        # Startup localisation replaces the first entry, so keep a per-instance
        # copy instead of mutating the module-level default route.
        self.checkpoints = list(CHECKPOINTS if checkpoints is None else checkpoints)
        ...
        # FROM MATTEO_GRANDE
        self.sign_points = np.loadtxt(data_path('sign_points.txt'), dtype=float)
        self.sign_types = np.loadtxt(data_path('sign_types.txt'), dtype=int)
        ...
        if not RANDOM_START and not RESUME and not self.path_only:
        ...
        while True:
            ...
            if not ALWAYS_DISTRUST_GPS or GPS_FOR_START_ONLY:
                ...
                if SIMULATOR_FLAG:
                    # get_closest_node_start historically accepts clockwise
                    # degrees and flips them into graph heading. Simulator yaw
                    # is already graph-frame, counter-clockwise radians.
                    startup_yaw_deg = -stable_yaw_deg
                else:
                    # Preserve the physical IMU's established 180-degree
                    # mounting correction, but apply it in the correct unit.
                    startup_yaw_deg = stable_yaw_deg + YAW_OFFSET
                print(
                    f"Current position: {curr_pos}, "
                    f"yaw: {stable_yaw_deg:.1f} deg"
                )
    ...
    def start_state(self):
        ...
        with open(state_path('remaining_checkpoints.txt'), "w") as f:
        ...
        self.path_follower.set_path(self.path_planner.path)
        self.path_following_command = None



        if self.path_only:
            # Isolate the controller test from mapped events and camera
            # detections. Normal autonomous mode still builds and processes the
            # complete event sequence below.
            self.events = [Event(
                nac.END_EVENT,
                dist=self.path_follower.route_length_m,
                point=self.path_planner.path[-1],
            )]
        else:
    ...
    def lane_following(self):  # LANE FOLLOWING ##############################
        if self.path_only:
            # Controller-isolation mode deliberately ignores stop lines,
            # crosswalks, signs, traffic, parking, tunnels and other event
            # transitions. It is intended only for simulator/closed-course
            # validation of graph localisation and checkpoint steering.
            self.activate_routines([
                nac.FOLLOW_LANE,
                nac.DRIVE_DESIRED_SPEED,
            ])
            if checkpoint_route_complete(self.path_following_command):
                self.activate_routines([])
                self.next_checkpoint()
                self.switch_to_state(nac.START_STATE)
            return
    ...
    # Event: End
    # UNUSED: Brain.lane_following_to_end has no production caller.
    ...
    # UNUSED: Brain.add_routines has no production caller.
    ...
    # UNUSED: Brain.switch_to_prev_state has no production caller.
    ...
    # UNUSED: Brain.get_frames_in_range has no production caller.
