# Purpose: Load perception models and detect lanes, stop lines, signs, and obstacles.
# Inputs: OpenCV camera frames and packaged ONNX/pickle models.
# Outputs: Lane errors, distances, class predictions, and debug imagery.
...
from brain_core.perception.stopline import detect_angle
from brain_core.common.resources import model_path

LANE_KEEPER_PATH = model_path('lane_keeper_small.onnx')
# LANE_KEEPER_PATH = "models/round_about8001.onnx"
# avg right -0.17356677295197556
DISTANCE_POINT_AHEAD = 0.35
CAR_LENGTH = 0.4

LANE_KEEPER_AHEAD_PATH = model_path('lane_keeper_ahead.onnx')
# LANE_KEEPER_AHEAD_PATH = "models/round_about8001.onnx"
INTERSECTION_NAVIGATOR_RIGHT = model_path('he_right.onnx')
INTERSECTION_NAVIGATOR_LEFT = model_path('he_left.onnx')
INTERSECTION_NAVIGATOR_FORWARD = model_path('he_straight.onnx')
ROUNDABOUT_NAVIGATOR_IN = model_path('he_right.onnx')
ROUNDABOUT_NAVIGATOR_ABOUT = model_path('about_dif.onnx')
ROUNDABOUT_NAVIGATOR_OUT = model_path('he_right.onnx')
# LANE_KEEPER_AHEAD_PATH = "models/round_in.onnx"
DISTANCE_POINT_AHEAD_AHEAD = 0.6

STOPLINE_ESTIMATOR_PATH = model_path('stopline_estimator.onnx')

STOPLINE_ESTIMATOR_ADV_PATH = model_path('stopline_estimator_advanced.onnx')
...
class Detection:
    ...
    def __init__(self) -> None:
        ...
        self.svm_model = pickle.load(open(
            model_path('obstacle_models', 'svm_' + self.kernel_type + '_' +
                       str(self.no_clusters) + '.pkl'), 'rb'))
        self.kmean_model = pickle.load(open(
            model_path('obstacle_models', 'kmeans_' + self.kernel_type + '_' +
                       str(self.no_clusters) + '.pkl'), 'rb'))
        self.scale_model = pickle.load(open(
            model_path('obstacle_models', 'scale_' + self.kernel_type + '_' +
                       str(self.no_clusters) + '.pkl'), 'rb'))
    ...
    # UNUSED: Detection.detect_roundabout_in has no production caller.
    ...
    # # SIGN DETECTION
    # UNUSED: Detection.tile_image has no production caller.
    ...
    # UNUSED: Detection.detect_sign_old has no production caller.
    ...
    """
    # UNUSED: Detection.classify_frontal_obstacle has no production caller.
    ...
    # UNUSED: Detection.classify_frontal_obstacle2 has no production caller.
