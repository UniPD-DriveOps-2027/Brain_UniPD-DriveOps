# Purpose: Provide image-processing utilities for stop-line geometry.
# Inputs: Camera image arrays and optional calibration/debug parameters.
# Outputs: Detected line angles, positions, distances, and diagnostic images.
...
class StopLine:
    ...
    def __init__(self, orig_frame):
        ...
        self.roi_points = np.float32([
            # Top-left corner
            # Top-left corner self.width//12, 2*self.height//3),
            (self.width//12, 3*self.height//5),
            # Bottom-left corner
            (self.width//12, self.height - self.height//24),
            # Bottom-right corner
            (self.width - self.width//12, self.height - self.height//24),
            # Top-right corner
        ...
        self.minpix = int((1/24) * self.height)

        # Best fit polynomial lines for left line and right line of the lane
        self.line_fit = None
        self.stopline_inds = None
        self.plotx = None
        self.line_fity = None
        self.linex = None
        self.liney = None
    ...
    # UNUSED: StopLine.calculate_histogram has no production caller.
    ...
    # UNUSED: StopLine.get_lane_line_indices_sliding_windows has no production caller.
    ...
    #
        ...
        # :return: Best fit lines for the left and right lines of the
                 # current lane
        # """
    ...
    #
            ...
            # # <++>
            # win_x_low = self.warped_frame.shape[1] - \
    ...
    #
            ...
            # stopline_inds.append(good_inds)
    #
            # # If you found > minpix pixels, recenter next
            # # window on mean position
            # minpix = self.minpix
            # if len(good_inds) > minpix:
                # y_current = int(np.mean(nonzeroy[good_inds]))
    #
        # # Concatenate the arrays of indices
        # stopline_inds = np.concatenate(stopline_inds)
    #
        # # Extract the pixel coordinates for the left and right lane lines
        # linex = nonzerox[stopline_inds]
        # liney = nonzeroy[stopline_inds]
    #
    ...
    #
            ...
            # plotx = np.linspace(0, frame_sliding_window.shape[0]-1,
                                # frame_sliding_window.shape[0])
    ...
    #
            ...
            # out_img[nonzeroy[stopline_inds],
                    # nonzerox[stopline_inds]] = [255, 0, 0]
    #
    ...
    def get_line_markings(self, frame=None):
        """
        Isolates lane lines.
        :param frame: The camera frame that contains the lanes we want to
                      detect
        ...
        # of the video frame. A high saturation value means the hue
        # color is pure.
        # We expect lane lines to be nice, pure colors
        # (i.e. solid white, yellow)
        # and have high saturation channel values.
        # s_binary is matrix full of 0s (black) and 255 (white)
        # intensity values
        ...
        # (white) intensity values # White in the regions with the richest red
    ...
    # UNUSED: StopLine.histogram_peak has no remaining production caller.
    # def histogram_peak(self):
        # """
        # Get the left and right peak of the histogram
    #
        # Return the x coordinate of the left histogram peak and the right
        # histogram peak.
        # """
...
def mag_thresh(image, sobel_kernel=3, thresh=(0, 255)):
    ...
    # mag is a 2D array .. number of rows x number of columns = number of pixs
